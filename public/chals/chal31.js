...
function push(num) {
  ...
  obj.text(num + "").font({size:20}).center(30,16).fill('white')
...
function pop() {
  if (stack.length == 0)
    return
  var el = stack.pop()
  el.animate(animationSpeed).attr({opacity:0.0}).dmove(70,0).afterAll(function(){el.remove()})
...
addBtn(opp, "+", function(){
  biop(function(a,b){return a + b})  
}, true)

addBtn(opp, "-", function(){
  biop(function(a,b){return b - a})  
})
...
addBtn(opp, "/", function(){
  if (stack.length > 0 && stack[stack.length-1].myValue != 0)
    biop(function(a,b){return Math.floor(b / a)})
  else
    alert("Keine Division durch 0")  
})
